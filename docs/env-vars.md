...
| --- | --- | --- | --- | --- |
| `ROM_WEAVER_LOG` | filter string | unset | `rom-weaver-cli/src/lib.rs` | Tracing filter spec (e.g. `rom_weaver_app=trace`); also honored via `RUST_LOG` when no explicit CLI log level is selected. |
