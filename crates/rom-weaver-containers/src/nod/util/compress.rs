...
impl DecompressionKind {
    pub fn from_wia(disc: &WIADisc) -> Result<Self> {
        ...
        match disc.compression() {
            ...
            comp => Err(Error::DiscFormat(format!(
                "Unsupported WIA/RVZ compression: {:?}",
                comp
            ))),
        }
    }
    ...
    fn decompress_raw(
        ...
        if in_pos != buf.len() {
            return Err(io::Error::new(
                ErrorKind::InvalidData,
                format!(
                    "LZMA decompression consumed {} of {} bytes",
                    in_pos,
                    buf.len()
                ),
            ));
