...
target/release/rom-weaver extract packages/rom-weaver-react/tests/fixtures/archives/one-rom.zip --out-dir /tmp/ex --checksum-rom crc32
...
# 2. drive it (separate shell, still inside packages/rom-weaver-react)
...
- `Cannot find package 'playwright'` → you ran the driver from the wrong cwd with a hand-rolled import, or the package's `node_modules` is absent. Run `npm --prefix packages/rom-weaver-react ci`.
