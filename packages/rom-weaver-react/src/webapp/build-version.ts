...
const VERSION_DISPLAY_BRANCH = GIT_BRANCH ? `.${GIT_BRANCH}` : "";
const APP_BUILD_VERSION = `${APP_VERSION}+${VERSION_BRANCH_PREFIX}${DIRTY_HASH ? `dirty.${DIRTY_HASH}` : COMMIT_HASH}`;
const APP_FOOTER_VERSION = `${APP_VERSION}${VERSION_DISPLAY_BRANCH}${
  DIRTY_HASH ? `.dirty#${DIRTY_HASH}` : `#${COMMIT_HASH}`
}`;
...
export { APP_BUILD_VERSION, APP_FOOTER_VERSION, RESOLVED_APP_BUILD_VERSION };
