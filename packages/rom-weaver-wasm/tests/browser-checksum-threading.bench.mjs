...
const ALGORITHMS = parseStringList(
  readEnvValue('ROM_WEAVER_WASM_BENCH_THREADING_ALGORITHMS')
    ?? defaultCsv(CHECKSUM_THREADING_DEFAULTS.algorithms),
);
const SIZES_MIB = parseIntegerList(
  readEnvValue('ROM_WEAVER_WASM_BENCH_THREADING_SIZES_MIB')
    ?? defaultCsv(CHECKSUM_THREADING_DEFAULTS.sizes_mib),
);
