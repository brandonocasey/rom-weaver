...
const IPS_CREATE_MAX_INPUT_BYTES = 0x01000000;
const IPS_EBP_BENCH_MAX_BYTES = IPS_CREATE_MAX_INPUT_BYTES - 1;
...
const SELECTED_COMMANDS = parseCommandFilter(
  readEnvValue('ROM_WEAVER_WASM_BENCH_COMMANDS') ?? DEFAULT_COMMANDS,
);
...
const SELECTED_CODEC_LABELS = parseOptionalCodecLabels(
  readEnvValue('ROM_WEAVER_WASM_BENCH_CODECS') ?? 'all',
);
const SOURCE_SIZE_BYTES = readPositiveIntEnv(
  'ROM_WEAVER_WASM_BENCH_SOURCE_MIB',
  DEFAULT_SOURCE_SIZE_MIB,
) * MIB;
const PATCH_SIZE_BYTES = readPositiveIntEnv(
  'ROM_WEAVER_WASM_BENCH_PATCH_SOURCE_MIB',
...
const NEEDS_CHECKSUM_ARCHIVE_SOURCES = SELECTED_COMMANDS.has('checksum')
  && (SELECTED_CHECKSUM_MODES.has('auto-extract') || SELECTED_CHECKSUM_MODES.has('archive-no-extract'));
...
const NEEDS_DISC_SOURCE = NEEDS_COMPRESS_SOURCE
  && SELECTED_CONTAINER_FORMATS.some((formatName) => DISC_COMPRESS_INPUT_FORMATS.has(formatName));
const NEEDS_PATCH_PAIR = (SELECTED_COMMANDS.has('patch-create') || SELECTED_COMMANDS.has('patch-apply'))
    ...
    for (const formatName of SELECTED_PATCH_FORMATS) {
      ...
      bench(
        ...
        async () => {
          ...
          if (!patchSource) {
            throw new Error(`patch source unavailable for format:${formatName}: ${patchSourceUnavailableReason(formatName)}`);
          }
        ...
        benchOptions({
          ...
          teardown: () => removeGuestPath(
            fixtureRootHandle,
            patchApplyOutputPath(formatName),
            { recursive: false },
          ),
        }),
      );
...
async function ensureIpsEbpPatchPairFixtures() {
  const limitedSize = Math.min(PATCH_SIZE_BYTES, IPS_EBP_BENCH_MAX_BYTES);
  await ensureGuestPseudoRandomFile(fixtureRootHandle, IPS_EBP_PATCH_ORIGINAL_PATH, limitedSize, {
    chunkSizeBytes: 4 * MIB,
    seed: 0x1BADB002,
  });
  await ensureGuestPseudoRandomFile(fixtureRootHandle, IPS_EBP_PATCH_MODIFIED_PATH, limitedSize, {
    chunkSizeBytes: 4 * MIB,
    seed: 0x1BADB002,
    mutate: true,
  });
...
async function ensureArchiveSource(formatName, codecLabel) {
  ...
  const codecCase = selectedCodecCasesForFormat(formatName)
    .find(([candidateLabel]) => candidateLabel === codecLabel);
  if (!codecCase) {
  ...
  if (!result.ok) {
    const reason = terminalLabelFromResult(result)
      || `failed to prepare archive source ${formatName} codec:${codecLabel}`;
...
async function archiveSourceCacheReady(sourcePath, markerPath) {
  ...
  benchLog(`archive cache check source:${sourceBytes ?? 'missing'} marker:${markerBytes ?? 'missing'} path:${sourcePath}`);
...
async function preparePatchSeedSources() {
  for (const formatName of SELECTED_PATCH_FORMATS) {
    ...
    if (!result.ok) {
      ...
      unavailablePatchSeedSources.set(
        formatName,
        terminalLabel || `failed to prepare patch seed for ${formatName}`,
      );
      continue;
    }
...
function defaultArchiveUnavailableReason(formatName) {
  const firstKey = selectedCodecCasesForFormat(formatName)
    .map(([codecLabel]) => archiveSourceKey(formatName, codecLabel))
    .find((key) => unavailableArchiveSources.has(key));
  if (!firstKey) return 'no prepared archive source';
  return unavailableArchiveSources.get(firstKey) ?? 'no prepared archive source';
}
...
function extractOutputDirPath(formatName, codecLabel) {
  return joinGuestPath(
    OUTPUT_ROOT,
    'extract',
    `${token(formatName)}-${token(codecLabel)}`,
  );
}
...
function isEmptyWarmupHook(mode) {
  return mode === 'warmup'
    && BENCH_OPTIONS.warmupTime === 0
    && BENCH_OPTIONS.warmupIterations === 0;
}
...
function romWeaverCompressArgs({
  inputPath,
  formatName,
  outputPath,
  threads,
