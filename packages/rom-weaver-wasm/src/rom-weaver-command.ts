...
export function normalizeRomWeaverRunRequest(
  ...
  const command = normalizeRomWeaverCommand(
    hasRequestShape ? commandOrRequest.command : commandOrRequest,
  );
  const baseOutput =
    hasRequestShape && isObjectRecord(commandOrRequest.output)
...
function normalizeRomWeaverPatchCommand(patchCommand: RomWeaverPatchCommand): RomWeaverCommand {
  ...
  const patchArgs = isObjectRecord(patchCommand.args) && !Array.isArray(patchCommand.args)
    ? { ...patchCommand.args }
    : {};
  switch (patchType) {
    case 'apply':
...
function replaceRomWeaverCommandArgs(
...
function clampRomWeaverBrowserThreadBudget(
  value: unknown,
  options: RomWeaverBrowserThreadRequestOptions,
): unknown {
...
function parseRomWeaverThreadBudgetCount(
  value: unknown,
  options: RomWeaverBrowserThreadRequestOptions,
): number | null {
  const maxThreads = normalizePositiveIntegerOption(options.maxThreads, 64);
